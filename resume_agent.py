...
def process_resume(resume_name, resume_rules_tool, input_folder, output_folder):
    ...
    with open(path.join(output_folder, outfile_name), "w", encoding="utf-8") as output_file:
        ...
        advanced_kw_analysis = analyze_keyword_counts(advanced_kw_counts, advanced_kw,
                                                      "optional advanced",
                                                      "potential advanced programming skills")
...
def main():
    ...
    # Ensure the files/paths exist
    if not Path(resume_path).exists():
        print(f"Cannot find the path '{resume_path}'")
        sys.exit(1)

    if not Path(output_folder).exists():
        print(f"Cannot find the path '{output_folder}'")
        sys.exit(1)

    # Separate pdf filename from the path
    resume_path_obj = Path(resume_path)
    input_folder = str(resume_path_obj.parent)
    resume_name = resume_path_obj.name

    print("***********************")
    print(f"Reviewing '{resume_name}'")
